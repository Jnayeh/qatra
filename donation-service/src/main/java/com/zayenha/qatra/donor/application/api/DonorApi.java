...
public class DonorApi implements EntityApi<DonorProfileEntity> {
    ...
    public Optional<DonorProfileDTO> findOptionalByUserId(Long userId) {
        return donorRepositoryPort.findByUserId(userId).map(DonorApi::toDTO);
    }

    public DonorProfileDTO findByUserId(Long userId) {
        return donorRepositoryPort.findByUserId(userId).map(DonorApi::toDTO).orElseThrow(() -> new NotFoundException(
